...
typename int32_t vertexAttribPosition	= 0;
typename int32_t vertexAttribNormal		= 1;
typename int32_t vertexAttribTexCoord	= 2;
...
void Chunk::populateBuffers()
{
	...
	glNamedBufferStorage(m_vb, sizeof(m_vertData[0]) * m_vertData.size(), m_vertData.data(), GL_DYNAMIC_STORAGE_BIT);

	glVertexArrayVertexBuffer(m_VAO, 0, m_vb, 0, sizeof(m_vertData[0]));
	...
	glVertexArrayAttribFormat(m_VAO, 1, 3, GL_FLOAT, GL_FALSE, offsetof(Vertex, norm));
...
void Chunk::generateGLBufferData()
{
	for (uint32_t i = 0; i < m_vertData.size(); i++)
	{
		...
		else 
		{
			...
			m_vertIndices.emplace_back(m_vertPos.size());
		}
		...
		else
		{
			...
			m_vertIndices.emplace_back(m_vertNorm.size());
		}
		...
		else
		{
			...
			m_vertIndices.emplace_back(m_vertTC.size());
		}
	}
}
